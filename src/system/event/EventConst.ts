
export const LOAD_COMPLETE: symbol = Symbol();
export const LOAD_ERROR: symbol = Symbol();

export const CALL_FRAME: symbol = Symbol();

//-------touch-------
export const TOUCH_BEGIN: symbol = Symbol();
export const TOUCH_END: symbol = Symbol();
export const TOUCH_MOVE: symbol = Symbol();
export const TOUCH_CANCEL: symbol = Symbol();
export const TOUCH_TAP: symbol = Symbol();
export const TOUCH_DOUBLE:symbol = Symbol();

//--------ui--------------
export const SIZE_CHANGE:symbol = Symbol();